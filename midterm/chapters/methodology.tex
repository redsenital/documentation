...
\subsection{Context-Aware Analysis Module}
...
\subsubsection{Static Context Extraction}

This stage performs offline HTML parsing using \textbf{BeautifulSoup4 (BS4)} and custom parsers. The system inspects the DOM to identify injection surfaces such as attributes, text nodes, script bodies, URL parameters, and form fields.

\begin{itemize}
    \item Extracts tag names, attributes, raw values, depth, parent, and sibling structure.
    \item Detects static flags (HTML-entity usage, URL encoding, JavaScript escaping).
    \item Produces a structured static context object for every potential injection point.
\end{itemize}

\subsubsection{Dynamic Context Extraction}

Static content alone fails to capture runtime behavior. Red Sentinel launches a headless browser using \textbf{Playwright} and records dynamic modifications triggered by JavaScript execution.

\begin{itemize}
    \item Detects \texttt{innerHTML} / \texttt{outerHTML} sinks.
    \item Logs event listeners (\texttt{onclick}, \texttt{onload}, custom JS handlers).
    \item Captures nodes created or modified at runtime.
    \item Records whether the context becomes dynamic and vulnerable after execution.
\end{itemize}
...
\begin{itemize}
    \item Attributes starting with \texttt{on*} are labeled as event-handler contexts.
    \item Any element inserted through \texttt{innerHTML} is marked as DOM-XSS.
    \item URL-based attributes may produce JavaScript protocol execution.
\end{itemize}

These deterministic rules form the foundation for supervised ML labels and validation.

\subsubsection{ML-Based Attack and Payload Classification}

To enhance generalization, Red Sentinel trains two \textbf{Random Forest} classifiers using the unified context features:

\begin{enumerate}
    \item Attack Class Model (e.g., event-handler, DOM-based, attribute-breakout)
    \item Payload Type Model (e.g., js-exec, event-transition, html-breakout)
\end{enumerate}

Contexts are encoded using one-hot vectors for tags and attributes, together with dynamic boolean features.  
The ML output refines the rule-based inference and improves robustness across diverse HTML structures.

\subsubsection{C-Script Construction}
...
\subsubsection{Context Dataset Construction}

The module automatically generates training data by storing each context and its inferred labels in \textbf{JSONL} format.  
This dataset combines:

\begin{itemize}
    \item BS4 static extraction output
    \item Playwright dynamic logs
    \item Rule-engine classifications
    \item Cleaned ML-ready feature vectors
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/transformer.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/activity.png}
